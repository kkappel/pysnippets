...
def main():
  ...
  parser.add_argument('-t', '--savetv', action='store_true', help='Print content as Videos from Save.tv')
...
def SaveTv():
  '''
  List all Files from SaveTv-Formated
  '''
  global query
  c.execute('SELECT file FROM files ORDER by file')
  query = c.fetchall()
  for col in query:
    fn = col[0]
    fr = fn.split('_')
